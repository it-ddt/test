...
def run(*args):
    ...
    if not words_ammount_entry.get():
        messagebox.showerror('Ошибка!', 'Не указано количество слов!')
        return
    if not wc_width_entry.get():
        messagebox.showerror('Ошибка!', 'Не указана ширина картинки!')
        return
    if not wc_height_entry.get():
        messagebox.showerror('Ошибка!', 'Не указана высота картинки!')
        return
    if not wc_margin_entry.get():
        messagebox.showerror('Ошибка!', 'Не указан отступ картинки!')
        return
    if not parts_of_speech:
        messagebox.showerror('Ошибка!', 'Не указаны части речи!')
        return
    if file_path_lbl['text'] == 'Выбирите исходный файл с текстом':
        messagebox.showerror('Ошибка!', 'Не указан исходный файл!')
        return
    ...
    messagebox.showinfo(
        'Готово!',
        f'Картинка сохранена в {destination_file} \n' +
        f'Всего слов в тексте: {len(text_analyser.words)} \n' +
...
words_ammount_entry.insert(0, '100')
